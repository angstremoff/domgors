...
- Инцидент 29.08.2026 «не работает регистрация на web»: `POST /auth/v1/signup` → `504 upstream request timeout` (~35с), юзер НЕ создаётся; при этом login (0.2с), REST (0.3с) и `auth/v1/settings` (`disable_signup:false`, `mailer_autoconfirm:false`) — быстрые и корректные, а `/recover` на несуществующий email (без отправки письма) — мгновенный 200. Диагноз: зависает синхронная SMTP-отправка письма подтверждения (невалидный custom SMTP / network drop). Чинится ТОЛЬКО в Supabase Dashboard: Authentication → SMTP Settings (вернуть built-in или починить креды/хост), проверить Auth Hooks и Logs. Код-митигация добавлена: `getAuthErrorMessage` распознаёт сетевые таймауты → `auth.serviceTemporarilyUnavailable`; `signUp` web (`AuthProvider`) и mobile (`AuthContext.register`) обёрнуты в `Promise.race`-таймаут 25с. Диагностический паттерн: anon-key публичен в JS-бандле сайта → signup можно дёргать curl'ом напрямую.
- Инцидент 31.08.2026: та же корневая причина (custom SMTP), но SMTP теперь не виснет, а быстро отказывает: `POST /auth/v1/signup` → 500 `unexpected_failure` / `Error sending confirmation email` (~0.5–1.3с), health/settings при этом 200 и быстрые. Диагноз подтверждён curl'ом напрямую. SMTP-провайдер: Adriahost (`budo31.adriahost.com:587`, STARTTLS, пользователь `admin@domgo.rs`, Exim). Сервер жив и TLS валиден (проверено извне), неверный пароль даёт ровно `535 Incorrect authentication data` → тот самый 500. Чинится только в Supabase Dashboard → Authentication → Emails → SMTP Settings: обновить пароль ящика (Reveal → сверить с cPanel) либо, если пароль верный, — хостинг блокирует облачные IP Supabase (поддержка Adriahost или переезд на Resend/Brevo). `getAuthErrorMessage` теперь маппит `Error sending … email` → `auth.serviceTemporarilyUnavailable`. Тестовые регистрации от диагностики: `regtest*@domgo-test.rs` в auth.users — можно удалить.
- Продолжение 31.08: (а) отключение custom SMTP НЕ возвращает регистрацию — built-in почта отклоняет все адреса вне команды проекта (`400 email_address_invalid`, даже на реальный существующий домен), для пользователей это тупик, custom SMTP обязан быть включён. (б) Неудачные signup'ы оставляют «полусозданного» юзера в auth.users: повторная регистрация той же почты даёт сырой `500 duplicate key … users_email_key`; лечится удалением юзера в Authentication → Users, в `getAuthErrorMessage` маппится в `auth.emailAlreadyExists`. (в) Пароль ящика admin@domgo.rs проверен извне — валиден, письмо уходит руками; значит если после повторного включения custom SMTP 500 сохраняется — хостинг блокирует облачные IP Supabase (тикет Adriahost или переезд на Resend). (г) Финал: форма Supabase перезаполнена верно (admin@domgo.rs, budo31.adriahost.com:587) и сохранена, но 500 остался; тем же паролем AUTH извне проходит → Exim Adriahost отвечает `535 Incorrect authentication data` на AUTH с AWS-IP Supabase (анти-спам ACL). Регистрация работала до 29.08 (последний юзер 28.08). Причина найдена: 17.08.2026 Adriahost завершил миграцию DNS Control инфраструктуры (тикет #306021, бесплатный пакет переведён на новые серверы) — после неё новый Exim стал отклонять SMTP AUTH с облачных IP. Решение: тикет в Adriahost (разрешить SMTP AUTH с IP дата-центров) либо миграция отправки на Resend/Brevo. (д) Итог 31.08 ~18:30 — регистрация восстановлена через Resend: домен domgo.rs верифицирован в Resend (DNS в DirectAdmin Adriahost: TXT `resend._domainkey` = DKIM-ключ, CNAME `rsend` → `rsend-euw1.forge.rmta.net.`, CNAME `send` → `send.forge.rmta.net.`; в DirectAdmin значениям CNAME обязательна точка в конце, иначе панель приклеивает `.domgo.rs`). Supabase SMTP Settings: host `smtp.resend.com` (НЕ smtp.resend.dev — не резолвится!), port 587, username `resend`, password = Resend API-ключ `re_…`, sender `admin@domgo.rs`. Лимит Resend free: 100 писем/день, 3000/мес. Adriahost `budo31.adriahost.com:587` для auth-почты больше не использовать (535 с облачных IP не чинился, тикет так и не отправлен); пароль ящика admin@domgo.rs светился в чате — сменить. Тестовые юзеры от диагностики (final*/resendcheck*/recheck*/savedcheck*/verify*/probe*/regtest*@…) в auth.users — удалить.
